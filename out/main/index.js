    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith(".gguf")) {
          ...
          const knownModel = AVAILABLE_MODELS.find((m) => m.filename === file);
          const displayName = knownModel ? knownModel.name : file;
          models.push({
            name: displayName,
            filename: file,
            path: fullPath,
...
class BinaryDownloader {
  ...
  static {
    this.RELEASE_API_URL = "https://api.github.com/repos/ggml-org/llama.cpp/releases/latest";
  }
  async downloadBinary(targetDir, onProgress) {
    ...
    try {
      ...
      const tempPath = path.join(electron.app.getPath("temp"), filename);
      ...
      if (filename.endsWith(".zip")) {
        ...
        child_process.execSync(`tar -xzf "${tempPath}" -C "${binDir}"`);
      }
  ...
  async getDownloadUrl() {
    ...
    if (platform === "win32") {
      ...
      patterns = arch === "arm64" ? ["bin-macos-arm64.tar.gz", "bin-macos-x64.tar.gz"] : ["bin-macos-x64.tar.gz", "bin-macos-arm64.tar.gz"];
    } else {
      patterns = [
